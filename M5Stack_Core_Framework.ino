...
#include "plugins/UartDevice_LoRaWAN.h"   // LoRa P2P — M5 LoRaWAN Unit US915 (RAK3172)
...
//  fw.addPlugin(new UartDevice_ModuleLLM(Serial2, 115200));  // offline AI text chat
  ...
  // LoRa P2P test — shares the one Port-C UART with the Module LLM, so to
  // use it COMMENT OUT the two lines above and uncomment this one (the
  // router's local-LLM tier then goes unused).  Set the radio params in
  // UartDevice_LoRaWAN.h / Config.h to match the SX1262 peer.
